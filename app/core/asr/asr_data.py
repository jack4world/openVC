...
class ASRData:
    ...
    def save(
        ...
        if save_path.endswith(".srt"):
            ...
            self.to_ass(
                save_path=save_path, style_str=ass_style, layout=layout,
    ...
    def _wrap_text(text: str, max_chars: int) -> str:
        """Insert ASS hard line breaks (\\N) so no line exceeds max_chars.

        Works for both CJK (each char = 1 unit) and Latin (splits on spaces).
        """
        import unicodedata
        lines, cur, cur_len = [], [], 0
        is_cjk = lambda c: unicodedata.east_asian_width(c) in ("W", "F")

        for ch in text:
            w = 2 if is_cjk(ch) else 1
            if cur_len + w > max_chars * 2 and cur:
                # Prefer to break at space for Latin text
                for i in range(len(cur) - 1, -1, -1):
                    if cur[i] == " ":
                        lines.append("".join(cur[:i]))
                        cur = cur[i + 1:]
                        cur_len = sum(2 if is_cjk(c) else 1 for c in cur)
                        break
                else:
    ...
    def to_ass(
        ...
        ass_content = (
            ...
            "; https://github.com/jack4world/openVC\n"
            "ScriptType: v4.00+\n"
        ...
        for seg in self.segments:
            ...
            translated = seg.translated_text
            if max_line_chars:
                original = self._wrap_text(original, max_line_chars)
                if translated:
                    translated = self._wrap_text(translated, max_line_chars)
